...
4. 在命名框中输入文件名，按回车或点击「创建」。例如选择 JSON 后，输入 `config` 或 `config.json` 都会生成 `config.json`。按 Esc 或点击「取消」不会创建文件。
5. 右键一级菜单还有 **使用 VS Code 打开**、**使用 IDEA 打开**、**复制完整路径**、**复制文件名**、**复制终端路径** 和 **打开右键新建…**。
...
也可以点击主窗口中的格式卡片，选择文件夹并命名后创建。关闭设置中的「创建前输入文件名」可恢复快速创建模式，直接使用默认名称。设置完成后可关闭窗口；下次从右键菜单创建时，主应用会自动启动。
...
- 创建前可输入文件名，默认名称预先选中，支持回车创建、Esc 取消和标准剪贴板快捷键。自动补齐所选扩展名，不重复添加已经输入的同名扩展名。
- 命名框实时校验空名称、路径分隔符、控制字符和过长名称。重名自动编号，例如 `config 2.json`，保留已有文件。
- 一级菜单提供三种复制方式。空白处复制当前文件夹，选中项目时复制选中项；完整路径和文件名按行分隔，终端路径使用 shell 引号并以空格分隔多个参数。
...
Finder 扩展运行在沙盒中，提供菜单并把新建及打开编辑器的请求交给包含它的应用。复制路径由扩展直接写入剪贴板，主应用退出时也可使用。主应用通过 `NSWorkspace` 接收固定格式的 URL 请求，使用 Foundation 创建文件，或通过 `NSWorkspace.open` 将选中的文件 URL 交给指定编辑器；不需要 AppleScript、辅助功能权限、完全磁盘访问权限或 shell 命令。打开请求仅接受预定义的 VS Code / IDEA，不接受任意可执行文件或命令行参数。
...
Finder 会跨进程重建菜单项，不能依赖 `NSMenuItem.representedObject` 传递请求。扩展使用唯一的 `tag` 关联本地请求快照，并保留多个最近菜单的请求，避免菜单重建后点击失效或目标目录串位。主应用收到冷启动的请求时，会等 AppKit 完成启动后再处理。多个命名请求按顺序显示，避免弹出相互嵌套的对话框或混用目标目录。
